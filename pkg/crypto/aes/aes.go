...
var (
	DefaultCode = []byte{
		234, 125, 205, 169, 221, 26, 113, 69,
		243, 209, 0, 252, 69, 169, 212, 53,
		200, 133, 73, 27, 64, 251, 221, 198,
		154, 174, 66, 28, 106, 39, 50, 53,
	}
...
func (a *AESGCM) Encrypt(plaintext []byte) (string, error) {
	...
	// 返回 base64 编码的字符串
	urlEncoded := base64.RawURLEncoding.EncodeToString(ciphertext)
	return urlEncoded, nil
}

func (a *AESGCM) EncryptUseStandardBase64(plaintext []byte) (string, error) {
	block, err := aes.NewCipher(a.key)
	if err != nil {
		return "", fmt.Errorf("failed to create cipher: %w", err)
	}
	...
	// 返回 base64 编码的字符串
	urlEncoded := base64.StdEncoding.EncodeToString(ciphertext)
	return urlEncoded, nil
}
...
// Decrypt 从 base64 编码的密文字符串解密数据.
func (a *AESGCM) DecryptUseStandardBase64(encodedCiphertext string) ([]byte, error) {
	// 解码 base64
